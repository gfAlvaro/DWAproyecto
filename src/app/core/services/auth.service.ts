...
export class AuthService {
  ...
  perfilActualizado$ = new Subject<void>();
  private apiUrl = '/api';
  ...
  login( email: string, password: string ): Observable<LoginResponse> {
    return this.http.post<LoginResponse>(
      ...
      tap(response => {
        ...
        localStorage.setItem( 'admin', JSON.stringify(response.administrador) );
      })
    );
  }

  loginCliente( email: string, password: string ): Observable<ClienteLoginResponse> {
  ...
  actualizarNombreCliente(nuevoNombre: string): void {
    const clienteActual = this.getCliente();
    if (clienteActual) {
