...
export class CrearProducto {
  ...
  private adminProductosService = inject(AdminProductosService);
  guardando = false;
  error = '';
  ...
  // Convierte el nombre del producto en un slug.
  generarSlug(): void {
  ...
  guardarProducto(): void {
    ...
    formData.append('nombreProducto', this.productoForm.controls.nombreProducto.value!);
    formData.append('slug', this.productoForm.controls.slug.value!);
    formData.append('descripcion', this.productoForm.controls.descripcion.value || '');
    formData.append('precio', String(this.productoForm.controls.precio.value));

    formData.append(
      'stock', (this.productoForm.controls.stock.value ?? 0).toString());
    ...
    this.adminProductosService.crearProducto(formData).subscribe({
      ...
      error: (error) => {
        ...
        this.guardando = false;
        if (error.status === 409) {
          this.error = 'Ya existe un producto con ese slug.';
        } else if (error.status === 400) {
          this.error = error.error?.message || 'Los datos enviados no son válidos.';
        } else {
          this.error = 'No se ha podido crear el producto.';
        }
      }
    });
  }
