...
export class EditarProducto implements OnInit {
  ...
  guardar(): void {
    ...
    formData.append('nombreProducto', datos.nombreProducto ?? '');
    formData.append('slug', datos.slug ?? '');
    formData.append('descripcion', datos.descripcion ?? '');
    formData.append('precio',String(datos.precio ?? ''));
    formData.append('stock',String(datos.stock ?? 0));
    ...
    this.adminProductosService.actualizarProducto(this.productoId, formData).subscribe({
      ...
      error: (error) => {
        ...
        this.error = error?.error?.mensaje || 'No se pudo actualizar el producto';
        this.guardando = false;
        this.cdr.detectChanges();
      }
    });
